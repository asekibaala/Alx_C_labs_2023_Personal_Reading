...
void add_at_end(struct node *head, int data){
    ...
    //creating a pointer of type struct node for moving the pointer and temp 
    //node to store data to be added at the end of the list
    struct node *ptr , *temp;
    ptr = head;

    //creating the temp node and allocating it a memory location
    temp = (struct node*)malloc(sizeof(struct node));

    //attaching data to the temp node
    temp->data = data;
    temp->link = NULL;
    ...
    ptr->link = temp;
    printf("You have sucessfully added new node!\n");
}
