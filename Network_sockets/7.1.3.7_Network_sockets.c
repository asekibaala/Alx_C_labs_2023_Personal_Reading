...
/*
besides the different inclusions and using the #pragma, we’ve also replaced all the write() invocations with send(), and read() with recv()
we’ve also replaced all close() functions with closesocket() invocations – you know the reason, don’t you?
and we’ve also added Winsock initialization and clean-up – can you see them?
